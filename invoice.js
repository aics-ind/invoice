var invoice = {
	...
	_onload : function(){
		...
		if(!indexedDB){
			...
			let req = indexedDB.open("AICS-invoice", 1);
	...
	ntow: function(n, t = true){
		/*var int = parseInt(n),
		float = parseInt((n.toFixed(2) - int ) * 100);

		var ans = [];

		var a = ['zero','one','two','three','four','five', 'six','seven','eight','nine','ten','eleven','twelve','thirteen','fourteen','fifteen','sixteen','seventeen','eighteen','ninteen'];
		var b = ['','twenty','thirty','fourty','fiftey','sixtey','seventy','eighty','ninety','hundrerd'];
		...
		function c(g){
			...
			if(a[g]) return a[g];
				...
				var q = g % 10,
					h = parseInt( g / 10 );
				return b[(h==0?1:h)-1] + ' ' + a[q];
			};
		};
		function d(){
			if(t){
				if (float !== undefined ) ans.push('and '+c(float)+' paisa only');
				ans.unshift('Rupees');
			};
			ans = ans.join().replace(/[,]+/gi, " ");
		};

			ans.unshift(c(int));
			int = parseInt(int/100);
		if(!int) {d();return ans;};
			ans.unshift(a[int%10]+' hundred and');
			int = parseInt(int/10);
		if(!int) {d();return ans;};
			ans.unshift(c(int)+' thousands');
			int = parseInt(int/100);
		if(!int) {d();return ans;};
			ans.unshift(c(int)+' lakhs');
			int = parseInt(int/100);
		if(!int) {d();return ans;};
			ans.unshift(c(int)+' crore');
			int = parseInt(int/100);
		if(!int) {d();return ans;};
			ans.unshift(ntow(int, false));	
			d(); return ans;*/
			return ntow(n);
	},
